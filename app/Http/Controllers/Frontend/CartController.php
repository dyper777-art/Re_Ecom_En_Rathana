...
class CartController extends Controller
{
    ...
    public function updateAll(Request $request)
    {
        ...
        if (!$request->has('quantities') || !is_array($request->quantities) || count($request->quantities) === 0) {
            return redirect()->back()->with('error', 'No items to update in the cart.');
        }
        ...
        foreach ($request->quantities as $id => $quantity) {
            ...
            if ($quantity > $product->stock) {
                return redirect()->back()->with('error', "Requested quantity for '{$product->name}' exceeds available stock.");
            }
